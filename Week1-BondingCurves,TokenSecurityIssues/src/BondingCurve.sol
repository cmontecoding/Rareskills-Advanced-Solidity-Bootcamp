...
contract BondingCurve is ERC20 {
    ...
    event TokensPurchased(address indexed buyer, uint256 amount, uint256 paid, uint256 newSupply);
    event TokensSold(address indexed seller, uint256 amount, uint256 received, uint256 newSupply);
    ...
    function purchaseTokens(uint256 _tokensToBuy) external payable withinGasLimit {
    ...
    function calculatePurchasePrice(uint256 _tokensToBuy) public view returns (uint256) {
        uint256 newSupply = supply + _tokensToBuy;
        return calculatePriceForSupply(supply, newSupply);
    }

    function calculateSalePrice(uint256 _tokensToSell) public view returns (uint256) {
    ...
    function calculatePriceForSupply(uint256 newSupply, uint256 prevSupply) internal view returns (uint256) {
        ...
        uint256 totalPrice = (initialPrice + initialPrice + (slope * (newSupply + prevSupply - 1))) * (prevSupply - newSupply) / 2;
        return totalPrice;
    }
