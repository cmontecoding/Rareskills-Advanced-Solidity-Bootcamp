...
contract DepositoorAttack is IERC721Receiver {
    ...
    /// @dev The first half of the tokens are claimed in the "legal" claim and the second half
    /// comes when re-entering and calling claimEarnings. This is possible because the depositTime
    /// is not updated before we enter the claimEarnings function. If the wait time from staking to
    /// attack is less than 5 days, it will take more than these 2 transactions and the attack will not
    /// work because the depositTime will be updated before the third claim.
